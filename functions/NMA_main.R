...
load_nma_packages <- function() {
    required_packages <- c("readxl", "dplyr", "tidyr", "stringr", "meta", "netmeta", "openxlsx", "ggsci", 
        "svglite", "systemfonts", "showtext", "sysfonts", "rsvg", "jpeg")
...
MAIN_OUTCOME_WORKBOOK_ALIASES <- list(`Physical function` = c("disability.xlsx", "physical_function.xlsx", 
    "physical function.xlsx"), `Pain-related fear avoidance` = c("pain-related_fear_avoidance.xlsx", 
    "pain-related fear avoidance.xlsx", "pain_fear_avoidance.xlsx"), `Pain intensity` = c("pain_intensity.xlsx", 
    "pain intensity.xlsx"), `Health-related quality of life` = c("health-related_quality_of_life.xlsx", 
    "health-related quality of life.xlsx", "quality_of_life.xlsx", "qol.xlsx"))
...
# Export publication figures without altering their aspect ratio.
export_nma_svg_as_jpg <- function(svg_file, width_in, dpi = 600, quality = 1, bg = "white") {
    if (!file.exists(svg_file)) {
        stop("Cannot create JPG because the source SVG does not exist: ", svg_file)
    }
    width_in <- suppressWarnings(as.numeric(width_in)[1])
    dpi <- suppressWarnings(as.numeric(dpi)[1])
    quality <- suppressWarnings(as.numeric(quality)[1])
    if (!is.finite(width_in) || width_in <= 0) {
        stop("width_in must be positive when creating a JPG.")
    }
    if (!is.finite(dpi) || dpi <= 0) {
        stop("dpi must be positive when creating a JPG.")
    }
    if (!is.finite(quality) || quality <= 0 || quality > 1) {
        stop("quality must be in the interval (0, 1].")
    }
    jpg_file <- sub("\\.svg$", ".jpg", svg_file, ignore.case = TRUE)
    if (identical(jpg_file, svg_file)) {
        jpg_file <- paste0(svg_file, ".jpg")
    }
    if (file.exists(jpg_file)) {
        remove_status <- unlink(jpg_file, force = TRUE)
        if (remove_status != 0 || file.exists(jpg_file)) {
            stop("Unable to remove the existing JPG file: ", jpg_file)
        }
    }
    target_width_px <- as.integer(round(width_in * dpi))
    rendered_bitmap <- rsvg::rsvg(svg = svg_file, width = target_width_px)
    jpeg::writeJPEG(image = rendered_bitmap, target = jpg_file, quality = quality, bg = bg)
    if (!file.exists(jpg_file) || is.na(file.info(jpg_file)$size) || file.info(jpg_file)$size <= 0) {
        stop("JPG file was not created: ", jpg_file)
    }
    message("600 dpi JPG saved from final SVG without changing aspect ratio: ", normalizePath(jpg_file, 
...
make_bmj_style_network_plot <- function(node_summary, edge_summary, sequence = NULL, file_prefix, plot_title = "", 
    ...
    network_width_in <- 7.2
    network_height_in <- 6.2
    ...
    open_nma_svg(filename = network_file, width_in = network_width_in, height_in = network_height_in, 
        pointsize = 12, bg = bg_color)
    ...
    network_jpg_file <- export_nma_svg_as_jpg(svg_file = network_file, width_in = network_width_in, dpi = 600, 
        quality = 1, bg = bg_color)
    invisible(list(svg_file = network_file, jpg_file = network_jpg_file, plot_data = coords, edge_plot_data = edge_summary))
}
...
make_bmj_style_forest_plot <- function(nma_vs_reference, node_summary = NULL, reference_group, file_prefix, 
    ...
    forest_width_in <- NMA_FOREST_WIDTH_IN
    open_nma_svg(filename = svg_file, width_in = forest_width_in, height_in = forest_height_in, pointsize = 12, 
        bg = bg_color)
    on.exit(abort_nma_svg(), add = TRUE)
    ...
    jpg_file <- export_nma_svg_as_jpg(svg_file = svg_file, width_in = forest_width_in, dpi = 600, quality = 1, 
        bg = bg_color)
    invisible(list(plot_data = plot_tab, svg_file = svg_file, jpg_file = jpg_file))
}

# Validate one outcome/time-window dataset, fit the prespecified frequentist
# network meta-analysis, and generate all tabular and graphical outputs.
run_frequentist_nma <- function(arm_file, time_name = c("post_intervention", "short_term", "mid_term", 
    ...
    if (connection$n.subnets != 1) {
        ...
        message("The network contains ", connection$n.subnets, " disconnected components, so a single standard NMA cannot be fitted; ", 
            "the direct-evidence network plots were still created as SVG and JPG: ", normalizePath(network_only$network_result$svg_file, 
                winslash = "/", mustWork = TRUE))
        return(invisible(list(analysis_status = "network_plot_only_disconnected", outcome = outcome_name, 
            ...
            network_plot_svg = network_only$network_result$svg_file, network_plot_jpg = network_only$network_result$jpg_file)))
    }
    ...
    node_order <- if (is.null(node_order)) {
        ...
        unique(canonicalise_nma_node(node_order))
    }
    forest_order <- if (is.null(forest_order)) {
        NULL
    }
    else {
        unique(canonicalise_nma_node(forest_order))
    }
    ...
    if (!file.exists(network_result$jpg_file)) {
        stop("High-resolution network JPG was not created: ", network_result$jpg_file)
    }
    message("Network plots saved:\n  SVG: ", normalizePath(network_result$svg_file), "\n  JPG: ", normalizePath(network_result$jpg_file))
    ...
    if (!file.exists(forest_result$jpg_file)) {
        stop("High-resolution forest-plot JPG was not created: ", forest_result$jpg_file)
    }
    message("Forest plots saved:\n  SVG: ", normalizePath(forest_result$svg_file), "\n  JPG: ", normalizePath(forest_result$jpg_file))
    ...
    result_object <- list(arm_data = arm_analysis, reported_smd_used = reported_smd_used, excluded_no_time_data = no_time_data, 
        ...
        pscore_table_excel_file = pscore_excel_file, network_plot_svg = if (!is.null(network_result)) {
            ...
            NA_character_
        }, network_plot_jpg = if (!is.null(network_result)) {
            network_result$jpg_file
        } else {
            NA_character_
            ...
            NA_character_
        }, forest_plot_jpg = if (!is.null(forest_result)) {
            forest_result$jpg_file
        } else {
            NA_character_
    ...
    cat("Network SVG: ", normalizePath(network_result$svg_file), "\n")
    cat("Network JPG: ", normalizePath(network_result$jpg_file), "\n")
    cat("Forest SVG: ", normalizePath(forest_result$svg_file), "\n")
    cat("Forest JPG: ", normalizePath(forest_result$jpg_file), "\n")
...
# Resolve legacy workbook names while allowing an explicit input path.
normalise_main_filename <- function(x) {
    x <- basename(trimws(as.character(x)))
    x <- sub("[[:space:]]*\\([0-9]+\\)(?=\\.xlsx$)", "", x, perl = TRUE, ignore.case = TRUE)
    gsub("[^a-z0-9]+", "", tolower(x))
}

find_main_outcome_workbook <- function(working_dir, outcome_name, arm_file = NULL) {
    working_dir <- normalizePath(working_dir, winslash = "/", mustWork = TRUE)
    if (!is.null(arm_file) && length(arm_file) == 1L && !is.na(arm_file) && nzchar(trimws(as.character(arm_file)))) {
        requested_file <- trimws(as.character(arm_file))
        ...
        if (!file.exists(requested_file)) {
            stop("The specified main-analysis data file does not exist: ", requested_file)
        }
        return(normalizePath(requested_file, winslash = "/", mustWork = TRUE))
    }
    outcome_display <- make_display_outcome_name(outcome_name)
    candidate_names <- MAIN_OUTCOME_WORKBOOK_ALIASES[[outcome_display]]
    if (is.null(candidate_names) || length(candidate_names) == 0L) {
        stop("No data-file mapping is defined for this outcome: ", outcome_display)
    }
    exact_candidates <- file.path(working_dir, candidate_names)
    exact_hit <- exact_candidates[file.exists(exact_candidates)]
    if (length(exact_hit) > 0L) {
        return(normalizePath(exact_hit[1], winslash = "/", mustWork = TRUE))
    }
    available_files <- list.files(working_dir, pattern = "\\.xlsx$", full.names = TRUE, ignore.case = TRUE)
    available_files <- available_files[!grepl("^~\\$", basename(available_files))]
    available_keys <- normalise_main_filename(available_files)
    for (candidate_name in candidate_names) {
        hit <- which(available_keys == normalise_main_filename(candidate_name))
        if (length(hit) > 0L) {
            return(normalizePath(available_files[hit[1]], winslash = "/", mustWork = TRUE))
        }
    }
    stop("The main-analysis data file could not be found: ", outcome_display, "\nTried file names: ", paste(candidate_names, 
        collapse = ", "), "\nCurrent R working directory: ", working_dir, "\nXLSX files in the current directory: ", 
...
resolve_main_arm_sheet <- function(workbook, arm_sheet = NULL) {
    available_sheets <- readxl::excel_sheets(workbook)
    if (length(available_sheets) == 0L) {
        stop("The main-analysis workbook has no readable sheets: ", workbook)
    }
    normalise_sheet_name <- function(x) {
        gsub("[^a-z0-9]+", "", tolower(trimws(as.character(x))))
    }
    requested_sheets <- if (!is.null(arm_sheet) && length(arm_sheet) == 1L && !is.na(arm_sheet) && nzchar(trimws(as.character(arm_sheet)))) {
        trimws(as.character(arm_sheet))
    }
    else {
        c("Sheet1", available_sheets[1])
    }
    available_keys <- normalise_sheet_name(available_sheets)
    for (requested_sheet in unique(requested_sheets)) {
        hit <- which(available_keys == normalise_sheet_name(requested_sheet))
        if (length(hit) > 0L) {
            return(available_sheets[hit[1]])
        }
    }
    stop("The requested sheet could not be found in the main-analysis workbook: ", arm_sheet, "\nAvailable sheets: ", paste(available_sheets, 
        collapse = ", "))
}
...
# Run one prespecified outcome at one follow-up window.
run_main_outcome_time <- function(outcome_name = NMA_SEVEN_OUTCOMES, time_name = c("post_intervention", "short_term", 
    ...
    if (is.null(output_dir)) {
        output_dir <- file.path(working_dir, "main", analysis_file_stub(outcome_name, time_name))
    }
    message("Main-analysis data: ", basename(master_workbook), " / ", arm_sheet, "; SMD: ", if (is.null(reported_smd_data)) 
        "not read"
    else "SMD sheet in the same workbook")
    run_frequentist_nma(arm_file = master_workbook, arm_sheet = arm_sheet, outcome_name = if (identical(outcome_name, 
        "Physical function")) 
        "Disability"
    else outcome_name, time_name = time_name, output_dir = output_dir, reported_smd_data = reported_smd_data, 
...
run_main_outcome <- function(outcome_name = NMA_SEVEN_OUTCOMES, working_dir = locate_nma_working_dir(), 
    ...
    run_four_time_windows(outcome_name, working_dir, master_workbook, arm_sheet, output_root, continue_on_error, 
        run_one = function(current_time, current_output_dir) {
            run_main_outcome_time(outcome_name = outcome_name, time_name = current_time, working_dir = working_dir, 
                master_workbook = master_workbook, arm_sheet = arm_sheet, reported_smd_sheet = reported_smd_sheet, 
                output_dir = current_output_dir)
        })
}

# Run all seven outcomes for the primary analysis.
run_main_analysis <- function(working_dir = locate_nma_working_dir(), master_workbook = NULL, reported_smd_sheet = "SMD", 
    output_root = file.path(working_dir, "main"), continue_on_error = TRUE) {
    out <- setNames(vector("list", length(NMA_SEVEN_OUTCOMES)), NMA_SEVEN_OUTCOMES)
    for (oo in NMA_SEVEN_OUTCOMES) {
        out[[oo]] <- run_main_outcome(outcome_name = oo, working_dir = working_dir, master_workbook = master_workbook, 
