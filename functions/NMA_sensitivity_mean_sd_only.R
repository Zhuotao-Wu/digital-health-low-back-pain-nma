# Sensitivity analysis restricted to mean and SD data.
# Scenario-specific data selection, model settings, and outputs are unchanged.
# Public entry point: run_mean_sd_sensitivity_analysis().
...
# Apply the same NMA model using studies classified as Mean+SD only; directly
# reported SMD records are deliberately excluded from this sensitivity analysis.
...
SENSITIVITY_EXCLUDED_STUDIES <- NULL
...
find_sensitivity_workbook <- function(working_dir, sensitivity_workbook = NULL) {
    ...
    if (length(xlsx_candidates) == 1) {
        message("The default sensitivity-analysis workbook was not found; ", "using the only candidate workbook in the directory: ", 
...
read_mean_sd_studies <- function(workbook) {
    sheets <- readxl::excel_sheets(workbook)
    hit <- which(normalise_nma_key(sheets) == "datatype")
    if (length(hit) == 0L) 
        stop("The Data type sheet was not found in the study-information workbook.")
    dat <- as.data.frame(readxl::read_excel(workbook, sheet = sheets[hit[1]]), stringsAsFactors = FALSE)
    keys <- normalise_nma_key(names(dat))
    type_hit <- which(keys == "type")
    if (length(type_hit) == 0L) 
        stop("The Type column was not found in the Data type sheet.")
    ids <- normalise_study_id(dat[[1]])
    type_key <- normalise_nma_key(dat[[type_hit[1]]])
    allowed <- unique(ids[!is.na(ids) & nzchar(ids) & type_key == "meansd"])
    message("Data-type filter: Type = Mean+SD; included ", length(allowed), " studies.")
    allowed
}

# Run one outcome/time window using Mean+SD studies only.
run_mean_sd_sensitivity_outcome_time <- function(outcome_name = NMA_SEVEN_OUTCOMES, time_name = c("post_intervention", "short_term", 
    ...
    allowed <- read_mean_sd_studies(master_workbook)
    arm_ids <- read_arm_study_ids(master_workbook, arm_sheet)
    excluded <- setdiff(arm_ids, allowed)
    if (is.null(output_dir)) {
        output_dir <- file.path(working_dir, "sens_meansd", analysis_file_stub(outcome_name, time_name))
    }
    message("Mean/SD-only sensitivity analysis: ", arm_sheet, "; excluded ", length(excluded), 
        " non-Mean/SD studies from this outcome; the SMD sheet was not used.")
    run_frequentist_nma(arm_file = master_workbook, arm_sheet = arm_sheet, outcome_name = if (identical(outcome_name, 
        "Physical function")) 
        "Disability"
    else outcome_name, time_name = time_name, output_dir = output_dir, exclude_studies = excluded, reported_smd_data = NULL, 
...
run_mean_sd_sensitivity_outcome <- function(outcome_name = NMA_SEVEN_OUTCOMES, working_dir = locate_nma_working_dir(), 
    master_workbook = NULL, arm_sheet = NULL, output_root = file.path(working_dir, "sens_meansd"), 
    ...
    run_four_time_windows(outcome_name, working_dir, master_workbook, arm_sheet, output_root, continue_on_error, 
        run_one = function(current_time, current_output_dir) {
            run_mean_sd_sensitivity_outcome_time(outcome_name = outcome_name, time_name = current_time, working_dir = working_dir, 
                master_workbook = master_workbook, arm_sheet = arm_sheet, output_dir = current_output_dir)
        })
}

# Run all seven outcomes for the Mean/SD-only sensitivity analysis.
run_mean_sd_sensitivity_analysis <- function(working_dir = locate_nma_working_dir(), master_workbook = NULL, output_root = file.path(working_dir, 
    "sens_meansd"), continue_on_error = TRUE) {
    out <- setNames(vector("list", length(NMA_SEVEN_OUTCOMES)), NMA_SEVEN_OUTCOMES)
    for (oo in NMA_SEVEN_OUTCOMES) {
        out[[oo]] <- run_mean_sd_sensitivity_outcome(outcome_name = oo, working_dir = working_dir, master_workbook = master_workbook, 
            output_root = output_root, continue_on_error = continue_on_error)
    }
    invisible(out)
}
