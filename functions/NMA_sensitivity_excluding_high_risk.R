...
# Public entry point: run_high_risk_sensitivity_analysis().
...
# Apply the same NMA model as the main analysis after excluding studies whose
# Overall Bias classification is High.
...
SENSITIVITY_WORKBOOK_NAME <- "sensitivity_analysis.xlsx"

SENSITIVITY_WORKBOOK_CANDIDATES <- SENSITIVITY_WORKBOOK_NAME

SENSITIVITY_EXCLUDED_STUDIES <- c("2", "3", "10", "13", "18", "20", "22", "23", "24", "25", "29", "33", 
    "34", "41", "42", "49", "51", "53", "55", "58", "59", "60", "65", "68", "69", "73", "80", "81")

SENSITIVITY_SHEET_ALIASES <- list(`Physical function` = c("Physical function", "Disability"), `Pain-related fear avoidance` = c("Pain-related fear avoidance", 
...
resolve_sensitivity_sheet <- function(workbook, outcome_name, arm_sheet = NULL) {
    ...
    candidate_sheets <- if (!is.null(arm_sheet) && length(arm_sheet) == 1L && !is.na(arm_sheet) && nzchar(trimws(arm_sheet))) {
        ...
        unique(c(outcome_display, SENSITIVITY_SHEET_ALIASES[[outcome_display]]))
    }
    ...
    stop("The outcome sheet could not be found in the sensitivity-analysis workbook: ", outcome_display, "\nTried sheet names: ", 
        paste(candidate_sheets, collapse = ", "), "\nAvailable sheets: ", paste(available_sheets, collapse = ", "))
}

find_sensitivity_workbook <- function(working_dir, sensitivity_workbook = NULL) {
    working_dir <- normalizePath(working_dir, winslash = "/", mustWork = TRUE)
    if (!is.null(sensitivity_workbook) && length(sensitivity_workbook) == 1 && !is.na(sensitivity_workbook) && 
        nzchar(trimws(sensitivity_workbook))) {
        requested_file <- trimws(as.character(sensitivity_workbook))
    ...
    exact_candidates <- file.path(working_dir, SENSITIVITY_WORKBOOK_CANDIDATES)
    ...
    if (length(xlsx_candidates) == 1) {
        message("The default sensitivity-analysis workbook was not found; ", "using the only non-SMD workbook in the directory: ", 
            basename(xlsx_candidates))
        return(normalizePath(xlsx_candidates, winslash = "/", mustWork = TRUE))
    }
    if (length(xlsx_candidates) > 1) {
        stop("Multiple possible sensitivity-analysis workbooks were found: ", paste(basename(xlsx_candidates), 
            collapse = ", "), "\nSpecify this argument when running the function: ", "sensitivity_workbook = \"your_file.xlsx\"")
    }
    stop("The sensitivity-analysis workbook was not found.\n", "Place the sensitivity-analysis workbook in: ", working_dir)
}

filter_reported_smd_for_sensitivity <- function(reported_smd_data, excluded_studies = SENSITIVITY_EXCLUDED_STUDIES) {
...
read_high_risk_studies <- function(workbook) {
    sheets <- readxl::excel_sheets(workbook)
    hit <- which(normalise_nma_key(sheets) == "riskofbias")
    if (length(hit) == 0L) 
        stop("The Risk of bias sheet was not found in the study-information workbook.")
    dat <- as.data.frame(readxl::read_excel(workbook, sheet = sheets[hit[1]]), stringsAsFactors = FALSE)
    keys <- normalise_nma_key(names(dat))
    risk_hit <- which(keys %in% c("overallbias", "overallriskofbias"))
    if (length(risk_hit) == 0L) 
        stop("The Overall Bias column was not found in the Risk of bias sheet.")
    ids <- normalise_study_id(dat[[1]])
    risk_key <- normalise_nma_key(dat[[risk_hit[1]]])
    excluded <- unique(ids[!is.na(ids) & nzchar(ids) & risk_key %in% c("high", "highrisk")])
    message("Risk-of-bias filter: excluded ", length(excluded), " studies.")
    excluded
}

# Run one outcome/time window after synchronised high-risk exclusions.
run_high_risk_sensitivity_outcome_time <- function(outcome_name = NMA_SEVEN_OUTCOMES, time_name = c("post_intervention", "short_term", 
    ...
    excluded <- read_high_risk_studies(master_workbook)
    reported_smd_data <- read_master_smd(master_workbook, reported_smd_sheet)
    reported_smd_data <- filter_reported_smd_ids(reported_smd_data, exclude_ids = excluded, label = "SMD filter for the high-risk-of-bias sensitivity analysis")
    if (is.null(output_dir)) {
        output_dir <- file.path(working_dir, "sens_rob", analysis_file_stub(outcome_name, time_name))
    }
    message("High-risk-of-bias sensitivity analysis: ", arm_sheet, "; only studies with Overall Bias = High were excluded; all other arm-level and SMD data were retained.")
...
run_high_risk_sensitivity_outcome <- function(outcome_name = NMA_SEVEN_OUTCOMES, working_dir = locate_nma_working_dir(), 
    master_workbook = NULL, arm_sheet = NULL, reported_smd_sheet = "SMD", output_root = file.path(working_dir, 
        "sens_rob"), continue_on_error = TRUE) {
    outcome_name <- canonicalise_requested_outcome(outcome_name[1])
    run_four_time_windows(outcome_name, working_dir, master_workbook, arm_sheet, output_root, continue_on_error, 
        run_one = function(current_time, current_output_dir) {
            run_high_risk_sensitivity_outcome_time(outcome_name = outcome_name, time_name = current_time, working_dir = working_dir, 
                master_workbook = master_workbook, arm_sheet = arm_sheet, reported_smd_sheet = reported_smd_sheet, 
                output_dir = current_output_dir)
        })
}

# Run all seven outcomes for the high-risk-of-bias sensitivity analysis.
run_high_risk_sensitivity_analysis <- function(working_dir = locate_nma_working_dir(), master_workbook = NULL, reported_smd_sheet = "SMD", 
    output_root = file.path(working_dir, "sens_rob"), 
    continue_on_error = TRUE) {
    out <- setNames(vector("list", length(NMA_SEVEN_OUTCOMES)), NMA_SEVEN_OUTCOMES)
    for (oo in NMA_SEVEN_OUTCOMES) {
        out[[oo]] <- run_high_risk_sensitivity_outcome(outcome_name = oo, working_dir = working_dir, master_workbook = master_workbook, 
